...
namespace Europlan.Common {
	public partial class PipeProductPlanner<ProductType, LayDistanceType, RimType> : Component, IProductPlanner where ProductType : Product, IPipeProduct<LayDistanceType, RimType> where LayDistanceType: struct where RimType: struct {

		public enum PipeProductMode {
			EVM_NONE,
			EVM_ADD_AREA,
			EVM_ADD_RZ,
			EVM_DEL_RZ,
		...
		public PipeProductPlanner() {
			InitializeComponent();

			TypeConverterAttribute tca = Attribute.GetCustomAttribute(typeof(LayDistanceType), typeof(TypeConverterAttribute)) as TypeConverterAttribute;
			if (tca != null) {
				System.Reflection.ConstructorInfo ci = Type.GetType(tca.ConverterTypeName).GetConstructor(new Type[] { });
				if (ci != null) {
					this.layDistanceConverter = (ci.Invoke(new object[] { }) as TypeConverter);
				}
			}
			tca = Attribute.GetCustomAttribute(typeof(RimType), typeof(TypeConverterAttribute)) as TypeConverterAttribute;
			if (tca != null) {
				System.Reflection.ConstructorInfo ci = Type.GetType(tca.ConverterTypeName).GetConstructor(new Type[] { });
				if (ci != null) {
					this.rimTypeConverter = (ci.Invoke(new object[] { }) as TypeConverter);
				}
			}
		}

		public PipeProductPlanner(IContainer container) {
			container.Add(this);
			InitializeComponent();
		...
		private PipeProductMode mode = PipeProductMode.EVM_NONE;
		...
		private TypeConverter layDistanceConverter = null;
		private TypeConverter rimTypeConverter = null;
		...
		public ProductType Product {
			...
			set {
				...
				if (this.product != null) {
					object[] attributes = this.product.GetType().GetCustomAttributes(typeof(ProductNameAttribute), true);

					if (attributes.Length > 0) {
						this.productTypeName = (attributes[0] as ProductNameAttribute).Name;
					} else {
						this.productTypeName = this.product.GetType().Name;
					}
				} else {
		...
		public PipeProductMode Mode {
			get { return this.mode; }
			set {
				this.mode = value;
				if (this.mode == PipeProductMode.EVM_ADD_RZ) {
					rzStart = Point2D.Zero;
				} else if (this.mode == PipeProductMode.EVM_ADD_AREA) {
		...
		private void connectedPlanPanel_KeyDown(object sender, KeyEventArgs e) {
			if (this.mode == PipeProductMode.EVM_ADD_RZ || this.mode == PipeProductMode.EVM_DEL_RZ || this.mode == PipeProductMode.EVM_DEL_RED || this.mode == PipeProductMode.EVM_SET_TEXT) {
		...
		public void PaintAfterPlanPannel(Graphics g, Matrix4D additionalTransformation, Point2D mousePositionInPlan, Point mousePositionInControl, bool export) {
			if (this.product != null && this.product.AssociatedRoom != null && this.product.AssociatedRoom.RoomCoordinates != null) {
				...
				if (coordsPickedSoFar.Count > 0 && inDesign) {
					...
					} else if (this.Mode == PipeProductMode.EVM_ADD_RED) {
						c = Color.Black;
						if (this.ConnectedPlanPanel != null && this.ConnectedPlanPanel.ColorMode == ColorMode.CM_BLACK_BG) {
							c = Color.White;
						}
				...
				if (this.product.TextBoxPosition != Point2D.Zero) {
					...
					if (this.product.PlannedCalculationComplete && product.PlannedLayDistance.HasValue) {
						az = null;
						if (this.layDistanceConverter != null) {
							az = this.layDistanceConverter.ConvertToString(product.PlannedLayDistance.Value);
						}
						if (az == null) {
							az = product.PlannedLayDistance.Value.ToString();
						}
					}
					...
					if (this.product.PlannedCalculationComplete && product.PlannedRimType.HasValue) {
						rz = null;
						if (this.rimTypeConverter != null) {
							rz = this.rimTypeConverter.ConvertToString(product.PlannedRimType.Value);
						}
						if (az == null) {
							rz = product.PlannedRimType.Value.ToString();
						}
					}
		...
		public bool PlannerClick(WW.Math.Point2D planPoint, System.Drawing.Point pointInControl, MouseButtons button) {
			if (this.Mode != PipeProductMode.EVM_ADD_AREA && this.Mode != PipeProductMode.EVM_ADD_RED && this.Mode != PipeProductMode.EVM_SET_TEXT && button == MouseButtons.Right) {
				rzStart = Point2D.Zero;
				this.Mode = PipeProductMode.EVM_NONE;
				this.connectedPlanPanel.InvalidateGraphics();
				return true;
			...
			if (this.Mode == PipeProductMode.EVM_ADD_AREA || this.Mode == PipeProductMode.EVM_ADD_RED) {
				List<Point2D> border = new List<Point2D>();
				if (this.Mode == PipeProductMode.EVM_ADD_AREA) {
					border = this.product.AssociatedRoom.RoomCoordinates;
				} else {
			...
			if (this.Mode == PipeProductMode.EVM_ADD_RZ && button == MouseButtons.Left) {
				if (rzStart == Point2D.Zero) {
			...
			} else if (this.Mode == PipeProductMode.EVM_DEL_RZ && button == MouseButtons.Left) {
				double distance = double.MaxValue;
			...
			} else if (this.Mode == PipeProductMode.EVM_SET_TEXT && button == MouseButtons.Left) {
