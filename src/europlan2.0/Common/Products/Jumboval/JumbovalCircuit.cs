...
namespace Europlan.Common {
	public class JumbovalCircuit : Circuit {

		private static readonly ILog log = LogManager.GetLogger(typeof(JumbovalCircuit));

		[XmlIgnore]
		public JumbovalProduct JumbovalProduct {
			get { return this.PlannedProduct.Product as JumbovalProduct; }
			set {
		...
		public double GetAreaRim(Nullable<Europlan.Common.JumbovalProduct.JumbovalRimType> rimType) {
			if (!rimType.HasValue) {
				return 0;
			}

			double bRz = ((float)JumbovalProduct.GetRimWidth(rimType.Value)) / 100;
		...
		public void Calculate(Europlan.Common.JumbovalProduct.JumbovalLayDistance layDistance, Nullable<Europlan.Common.JumbovalProduct.JumbovalRimType> rimType) {
			if (layDistance == JumbovalProduct.JumbovalLayDistance.NONE) {
				c_Qh2oHeat = 0;
			...
			double c = JumbovalProduct.ConfigC;
			double v = JumbovalProduct.ConfigV;
			double rho = JumbovalProduct.ConfigRho;
			double ag = JumbovalProduct.ConfigAgActivated ? JumbovalProduct.ConfigAg : 1;
			...
			double rLambdaB = this.JumbovalProduct.PlannedInsideConstructionRValue;
			double rLambdaIns = this.JumbovalProduct.PlannedOutsideConstructionRValue;

			double factor = (this.JumbovalProduct.PlannedFloorConstruction != null && (this.JumbovalProduct.PlannedFloorConstruction.Type == ConstructionTypeManager.Instance.GetConstructionTypeById(ConstructionTypeManager.CT_STD_TROCKEN) || this.JumbovalProduct.PlannedFloorConstruction.Type == ConstructionTypeManager.Instance.GetConstructionTypeById(ConstructionTypeManager.CT_USER_TROCKEN))) ? JumbovalProduct.ConfigFaktorTrockenkonstruktion : 1;
			...
			bool calculateWithRim = rimType.HasValue && (rimLength - this.rimCorners * JumbovalProduct.GetRimWidth(rimType.Value) / 100 > 0);
			this.c_areaRz = 0;
			this.c_pipeLengthRz = 0;
			if (calculateWithRim) {
				this.c_areaRz = this.GetAreaRim(rimType);
				this.c_pipeLengthRz = this.c_areaRz * JumbovalProduct.GetPipeLengthPerSqm(JumbovalProduct.GetRimLayDistance(rimType.Value));  // Rohrlänge der Randzone berechnen
			}
			if (this.c_areaRz > aFbh) {
				this.c_areaRz = aFbh;
			}
			this.c_areaAz = aFbh - this.c_areaRz;                                                      // Fläche der Aufenthaltszone berechnen
			this.c_pipeLengthAz = (this.c_areaAz + this.areaReduced / 2) * JumbovalProduct.GetPipeLengthPerSqm(layDistance);                           // Rohlänge der Aufenthaltszone berechnen

			{ // Heizlastberechnung
				double distributorVorlaufTemp;
				double distributorRuecklaufTemp;
				this.JumbovalProduct.GetHeatFlow(out distributorVorlaufTemp, out distributorRuecklaufTemp);
				...
				if (calculateWithRim) {
					...
					dThetaRz = en1264.Heizmitteluebertemperatur(this.c_thetaVRzHeat, this.c_thetaRRzHeat, this.JumbovalProduct.AssociatedRoom.RoomHeatTemperature);
					//                                                                        // Heizmittelübertemperatur der Randzone berechnen
				}
				double dThetaAz = en1264.Heizmitteluebertemperatur(this.c_thetaVAzHeat, this.c_thetaRAzHeat, this.JumbovalProduct.AssociatedRoom.RoomHeatTemperature);
				...
				this.c_floorTempAzHeat = en1264.OberflaechenTemperatur(this.c_qAzHeatPerSqm, alphaFbh, this.JumbovalProduct.AssociatedRoom.RoomHeatTemperature);
				if (calculateWithRim) {
					this.c_floorTempRzHeat = en1264.OberflaechenTemperatur(this.c_qRzHeatPerSqm, alphaFbh, this.JumbovalProduct.AssociatedRoom.RoomHeatTemperature);
				} else {
			...
			{ // Kühllastberechnung
				double distributorVorlaufTemp;
				double distributorRuecklaufTemp;
				this.JumbovalProduct.GetCoolFlow(out distributorVorlaufTemp, out distributorRuecklaufTemp);
				...
				if (calculateWithRim) {
					...
					dThetaRz = en1264.Heizmitteluebertemperatur(this.c_thetaVRzCool, this.c_thetaRRzCool, this.JumbovalProduct.AssociatedRoom.RoomCoolTemperature);
					//                                                                        // Heizmittelübertemperatur der Randzone berechnen
				}
				double dThetaAz = en1264.Heizmitteluebertemperatur(this.c_thetaVAzCool, this.c_thetaRAzCool, this.JumbovalProduct.AssociatedRoom.RoomCoolTemperature);
				//                                                                            // Heizmittelübertemperatur der Aufenthaltszone berechnen
				...
				this.c_floorTempAzCool = en1264.OberflaechenTemperatur(this.c_qAzCoolPerSqm, alphaFbk, this.JumbovalProduct.AssociatedRoom.RoomCoolTemperature);
				if (calculateWithRim) {
					this.c_floorTempRzCool = en1264.OberflaechenTemperatur(this.c_qRzCoolPerSqm, alphaFbk, this.JumbovalProduct.AssociatedRoom.RoomCoolTemperature);
				} else {
