...
namespace Europlan.Common {
	public partial class PlannedModulKlimaBoden20ProductPanel : UserControl, IEditorUserControl {
		private PlannedProduct product = null;

		private bool gridContentChanged = false;
		private bool updateOngoing = false;

		public PlannedModulKlimaBoden20ProductPanel() {
			InitializeComponent();
		...
		private void SetLanguage() {
			...
			this.lblHk.Text = EuroplanRes.PlannedModulKlimaBoden20ProductPanel_Heizkreis + " 1:";
			...
			this.label2.Text = EuroplanRes.PlannedModulKlimaBoden20ProductPanel_SummeVerbindungsleitungen; //"Summe Verbindungsleitungen:"
			this.label12.Text = EuroplanRes.PlannedModulKlimaBoden20ProductPanel_Heizkreis; //"Heizkreise"
			this.label13.Text = EuroplanRes.PlannedModulKlimaBoden20ProductPanel_Teilflaechen; //"Teilflächen"
			this.label14.Text = EuroplanRes.PlannedModulKlimaBoden20ProductPanel_ErklaerungVerbindeleitungen; //"(Pro Winkel zusätzlich 1m Verbindungsleitung)"
			this.label1.Text = EuroplanRes.PlannedModulKlimaBoden20ProductPanel_ParalleleReihen; //"Parallele Reihen im Heizkreis"
		...
		public void UpdateControl(bool resetUserInterface) {
			...
			if (this.product != null) {
				(this.product.Product as ModulKlimaBoden20Product).ConfigureProduct(this.product.RequestedHeatLoad, this.product.RequestedCoolLoad, this.product.CalculateHeat, this.product.CalculateCool, false);
				this.errorMsg = this.product.Product.LastErrorMessage;
			}
			this.UpdateControl(FieldEnum.NONE);
		}

		private ModulKlimaBoden20Circuit selectedCircuit = null;
		private ModulKlimaBoden20SubArea selectedSubArea = null;
		...
		private void UpdateControl(FieldEnum skipFields) {
			...
			if (this.product != null) {
				...
				this.selectedCircuit = mdProduct.PlannedCircuits[this.lstCircuits.SelectedIndex] as ModulKlimaBoden20Circuit;
				if ((skipFields & FieldEnum.SUBAREA) == FieldEnum.NONE) {
					if (this.selectedCircuit.SubAreas.Count == 0) {
						this.selectedCircuit.SubAreas.Add(new ModulKlimaBoden20SubArea());
					}
					this.lstSubarea.Items.Clear();
					int count = 1;
					foreach (ModulKlimaBoden20SubArea sa in this.selectedCircuit.SubAreas) {
				...
				lblAvgqHeat.Text = this.product.Product.PlannedCircuits.Count > lstCircuits.SelectedIndex ? Math.Round((this.product.Product.PlannedCircuits[lstCircuits.SelectedIndex] as ModulKlimaBoden20Circuit).C_QHeatPerSqm, 2).ToString() : "0";
				...
				lblAvgqCool.Text = this.product.Product.PlannedCircuits.Count > lstCircuits.SelectedIndex ? (-1.0 * Math.Round((this.product.Product.PlannedCircuits[lstCircuits.SelectedIndex] as ModulKlimaBoden20Circuit).C_QCoolPerSqm, 2)).ToString() : "0";
		...
		private void btnInsulationConstruction_Click(object sender, EventArgs e) {
			...
			form.SelectedConstruction = (this.product.Product as ModulKlimaBoden20Product).PlannedInsulationConstruction;
			if (form.ShowDialog() == DialogResult.OK) {
				if (form.SelectedConstruction != null) {
					(this.product.Product as ModulKlimaBoden20Product).PlannedInsulationConstruction = form.SelectedConstruction;
					this.product.Product.ConfigureProduct(this.product.RequestedHeatLoad, this.product.RequestedCoolLoad, this.product.CalculateHeat, this.product.CalculateCool, false);
					this.errorMsg = this.product.Product.LastErrorMessage;
					this.UpdateControl(FieldEnum.NONE);
		...
		private void numRoomTemperatureBelowHeat_ValueChanged(object sender, EventArgs e) {
			if (ignoreRoomTemperatureBelowHeat == 0) {
				(this.product.Product as ModulKlimaBoden20Product).PlannedRoomTemperatureBelowHeat = (float)this.numRoomTemperatureBelowHeat.Value;
				this.product.Product.ConfigureProduct(this.product.RequestedHeatLoad, this.product.RequestedCoolLoad, this.product.CalculateHeat, this.product.CalculateCool, false);
				this.errorMsg = this.product.Product.LastErrorMessage;
		...
		private void numRoomTemperatureBelowCool_ValueChanged(object sender, EventArgs e) {
			if (ignoreRoomTemperatureBelowCool == 0) {
				(this.product.Product as ModulKlimaBoden20Product).PlannedRoomTemperatureBelowCool = (float)this.numRoomTemperatureBelowCool.Value;
				this.product.Product.ConfigureProduct(this.product.RequestedHeatLoad, this.product.RequestedCoolLoad, this.product.CalculateHeat, this.product.CalculateCool, false);
				this.errorMsg = this.product.Product.LastErrorMessage;
