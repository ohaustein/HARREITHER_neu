...
namespace Europlan.Common
{
    public partial class ModulKlimaBoden20Planner : Component, IProductPlanner
    {
        ...
        private void DrawDxfModule(KlimaFlaechenModul.ModulTypeEnum type, Nullable<KlimaFlaechenModul.ModulOrientationEnum> orientation, Point2D position, Matrix4D additionalTransformation, WW.Cad.Model.DxfModel model, DxfLayer layer, bool bottomUp, Color circuitColor, double rotation)
        {
            ...
            KlimaFlaechenModul.ModulOrientationEnum? orientationForDrawing = orientation;
            if (Europlan.Common.Product.ConfigViewGrundriss && orientation.HasValue)
            {
                orientationForDrawing = (orientation == KlimaFlaechenModul.ModulOrientationEnum.ORIENTATION_LEFT) ? KlimaFlaechenModul.ModulOrientationEnum.ORIENTATION_RIGHT : KlimaFlaechenModul.ModulOrientationEnum.ORIENTATION_LEFT;
            }
