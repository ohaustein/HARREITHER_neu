...
namespace Europlan.Common {
	public partial class ModulKlimaDeckePlanner : Component, IPlanner, IProductPlanner {
		...
		public void DrawModule(KlimaFlaechenModul.ModulTypeEnum type, Nullable<KlimaFlaechenModul.ModulOrientationEnum> orientation, Point2D position, Matrix4D additionalTransformation, Graphics g, bool bottomUp, bool highlight, Color circuitColor, bool highlightInput, bool highlightOutput, bool cadPlan) {
            ...
            if ((cadPlan || type == KlimaFlaechenModul.ModulTypeEnum.MODUL_60_60 || type == KlimaFlaechenModul.ModulTypeEnum.MODUL_60_60B) ^ Europlan.Common.Product.ConfigViewGrundriss)
            {
		...
		private void DrawDxfModule(KlimaFlaechenModul.ModulTypeEnum type, Nullable<KlimaFlaechenModul.ModulOrientationEnum> orientation, Point2D position, Matrix4D additionalTransformation, WW.Cad.Model.DxfModel model, DxfLayer layer, bool bottomUp, Color circuitColor) {
            ...
            // ?
            if (((type == KlimaFlaechenModul.ModulTypeEnum.MODUL_60_60 || type == KlimaFlaechenModul.ModulTypeEnum.MODUL_60_60B) && bottomUp) ^ Europlan.Common.Product.ConfigViewGrundriss)
            {
                if (orientation == KlimaFlaechenModul.ModulOrientationEnum.ORIENTATION_LEFT) {
