...
namespace Europlan.Application
{
    public partial class MainForm : Form
    {
        ...
        private Boolean RunUpdateOnClose = false;


        private string projectFileName = null;
        private Project currentProject = null;

        private static readonly string defaultTitle = "Europlan 3.1";
        ...
        private void Form_Shown(object sender, EventArgs e)
        {
            try
            {
                this.bgwUpdateCheck.RunWorkerAsync();
            }
            catch { }
        }
        ...
        private String GetSetupLanguage()
        {
            const String defaultLanguage = "de";
            try
            {
                var key = Microsoft.Win32.Registry.LocalMachine.OpenSubKey("Software\\bluesource\\Europlan3.1");
                if (key != null)
                {
                    var setupLanguage = key.GetValue("SetupLanguage") as string;
        ...
        private void MainForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                if (RunUpdateOnClose)
                {
                    ...
                    {
                        UpdateInfo.Default.Execute();
                    }
                }
            }
            catch (Exception ex)
            {
                FormException.Anzeigen(ex);
            }
        }
        ...
        private void myProject_ProjectSaved(object sender)
        {
            projectUnsaved = false;
            UpdateTitle();
            Project.Instance.InitializeTreeView(this.projectTree);
        ...
        private void menUpdateInfo_Click(object sender, EventArgs e)
        {
            try
            {
                if (UpdateInfo.Default != null)
                {
                    var caption = "Update";
                    var text = $"Update auf Version {UpdateInfo.Default.Version.ToShortString()} verfügbar";
                    MessageBox.Show(text, caption, MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("Keine Update-Informationen verfügbar", String.Empty, MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
        ...
        private void menUpdateNow_Click(object sender, EventArgs e)
        {
            try
            {
                if (UpdateInfo.Default != null)
                {
                    var message = "Die Anwendung wird und geschlossen und im Anschluss daran wird das Update durchgeführt.\n\nBitte schließen Sie vor dem Beender der Anwendung all Ihre Arbeit.";
                    var result = MessageBox.Show(message, String.Empty, MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
                    if (result == DialogResult.OK)
                    {
        ...
        private void menUpdateLater_Click(object sender, EventArgs e)
        {
            try
            {
                if (UpdateInfo.Default != null)
                {
                    RunUpdateOnClose = !RunUpdateOnClose;
                    this.menUpdateLater.Checked = RunUpdateOnClose;
                }
                else
                {
                    MessageBox.Show("Keine Update-Informationen verfügbar", String.Empty, MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void menUpdateSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (UpdateInfo.Default != null)
                {
                    using (var updateSaveDialog = new SaveFileDialog()
                    {
                        DefaultExt = ".msi",
                        FileName = UpdateInfo.Default.DefaultFilename,
                        Filter = "*.msi|Setup-Dateien|*.*|Alle Dateien",
                        Title = "Bitte wählen Sie aus, an welchem Ort das Setup gespeichert werden soll"
                    }
                    )
                    {
                        if (updateSaveDialog.ShowDialog(this) == DialogResult.OK)
                        {
                            UpdateInfo.Default.SaveTo(updateSaveDialog.FileName);
                        }
                    }
                }
                else
        ...
        private void bgwUpdateCheck_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            {
                var downloadResultCollection = Controller.RunDownload(checkResultCollection)
                        .Where(item => item.WasSuccessful.GetValueOrDefault(false))
                        .OrderByDescending(item => item.Version);
                var downloadResultCount = downloadResultCollection.Count();
                if (downloadResultCount > 0)
                {
                    var latestDownloadedItem = downloadResultCollection.First();
                    UpdateInfo.SetDefault(latestDownloadedItem.LocalFilePath, setupLanguage, latestDownloadedItem.Version);
                }
            }
        }

        private void bgwUpdateCheck_RunWorkerCompleted(object sender, System.ComponentModel.RunWorkerCompletedEventArgs e)
        {
            if (e.Error == null)
            {
                if (UpdateInfo.Default != null)
                {
                    this.menUpdate.Visible = true;
                }
                else
                {
                    this.menUpdate.Visible = false;
                }
            }
            else
            {
